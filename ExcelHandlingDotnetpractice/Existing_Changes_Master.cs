...
namespace ExcelHandlingDotnetpractice
{
    ...
    {
        public static void Existing_changes_Master(string filePath, string destinationFolder)
        {
            try
            {
                    ...
                    using (var outputPackage = new ExcelPackage())
                    {
                        ...
                        for (row = 2; row <= lastRow; row++)
                        {
                            ...
                            if (string.IsNullOrEmpty(bgColor.Rgb) || bgColor.Rgb.Equals("FFFFFF", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                outputWorksheet.Cells[row7, 8].Value = outputWorksheet.Cells[row7, 3].GetValue<string>() + " " + outputWorksheet.Cells[row7, 5].GetValue<string>();
                        ...
                        Console.WriteLine("Existing_Changes_Master Excel file created successfully!");
                    }
                }
            }
            catch (Exception ex)
