...
namespace ExcelHandlingDotnetpractice
{
    ...
    {
        public static void NewJoinee_Master(string filePath, string destinationFolder)
        {
            ...
            try
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    using (var outputPackage = new ExcelPackage())
                    {
                        ...
                        outputWorksheet.Cells[1, 83].Value = "Employee Grade";
                        ...
                        for (row = 2; row <= lastRow; row++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(bgColor.Rgb) && !bgColor.Rgb.Equals("FFFFFF", StringComparison.OrdinalIgnoreCase))
                            {
                               ...
                               // outputWorksheet.Cells[row, 95].Value = PreferredName;
                                ...
                                //    if (HRID.Equals(inputcostcentersheet.Cells[i, 1].GetValue<string>())) {
                                //        outputWorksheet.Cells[row7, 54].Value = inputcostcentersheet.Cells[i, 2].GetValue<string>();
                                //    }
                                //}
                                var columnRange = outputWorksheet.Cells[1, 54, row7, 54];
                                ...
                                for (row2 = 2; row2 <= BenefeciarieslastRow; row2++)
                                {
                                    ...
                                    if (HRID.Equals(id))
                                    {
                                        outputWorksheet.Cells[row7, 95].Value = BenefeciariesDataSheet.Cells[row2, primarynameasperbank].GetValue<string>();
